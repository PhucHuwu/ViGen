...
class Screenwriter:
    ...
    async def write_script_based_on_story(
        ...
        class WriteScriptBasedOnStoryResponse(BaseModel):
            script: List[Union[str, Dict[str, Any]]] = Field(
                ...,
                description="The script based on the story. Each element is a scene. Can be a string or a structured object."
            )
        ...
        try:
            ...
            raw_script = parsed_response.script
        except Exception as e:
            logging.warning(f"Failed to parse script with Pydantic: {e}. Trying to parse as raw JSON or returning raw content.")
            # Fallback output handling if Pydantic fails completely
            # attempting manual json load if possible
            import json
            try:
                # Try to find json-like list in content
                content = response.content
                start = content.find('[')
                end = content.rfind(']')
                if start != -1 and end != -1:
                    raw_script = json.loads(content[start:end+1])
                else:
                    raise ValueError("No JSON list found")
            except:
                # Fail hard if we can't get a list
                raise e

        # Post-process: Ensure everything is a string
        final_script = []
        for item in raw_script:
            if isinstance(item, str):
                final_script.append(item)
            elif isinstance(item, dict):
                # Convert dict to script format string
                scene_header = item.get('scene', 'SCENE')
                setting = item.get('setting', '')
                action = item.get('action', '')
                dialogue = item.get('dialogue', '')  # Sometimes models put dialogue separately

                # Format:
                # SCENE HEADER - SETTING
                # Action...
                #
                # CHARACTER
                # Dialogue...

                formatted_scene = f"**{scene_header}** - {setting}\n\n{action}"
                if dialogue:
                    formatted_scene += f"\n\n{dialogue}"

                final_script.append(formatted_scene)
            else:
                final_script.append(str(item))
