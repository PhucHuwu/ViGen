    ...
    async def generate_single_image(
        ...
        **kwargs,
        ...
        if not candidate.content or not candidate.content.parts:
            # Check for safety ratings/finish reason if available
            finish_reason = getattr(candidate, 'finish_reason', 'UNKNOWN')
            safety_ratings = getattr(candidate, 'safety_ratings', 'UNKNOWN')
            logging.error(f"Generation failed. Finish Reason: {finish_reason}. Safety Ratings: {safety_ratings}")
            raise ValueError(f"Gemini API returned no content. Reason: {finish_reason}")
